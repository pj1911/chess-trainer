...
function tryUserMove(from, to, promotion) {
  // chess.js 0.10.3 expects {from,to,promotion?}
  const moveObj = promotion ? { from, to, promotion } : { from, to };
  const mv = chess.move(moveObj);
  if (!mv) return false;

  // Ensure board matches exactly (promotion, en passant, castling, etc)
...
function setReviewPly(plyIndex) {
  ...
  const humanMoveIndex = Math.max(0, review.currentPly - 1);
  ui.navLabel.textContent = review.currentPly === 0 ? "Start" : `After ply ${review.currentPly}`;

  // highlight selected card
  review.moveRows.forEach((el, idx) => el.style.outline = idx === humanMoveIndex ? "1px solid #3a4666" : "none");
}
...
async function runAnalysisAndRender({ autoSavePrompt = true } = {}) {
  ...
  for (let ply = 0; ply < historyVerbose.length; ply++) {
    ...
    card.className = "analysisMove";
    card.dataset.ply = String(ply + 1);
    card.innerHTML = `
      <div class="analysisMoveTop">
        <div class="analysisMoveMain">
          <span class="badge ${badgeClass(label)}">${label}</span>
          <strong>${Math.floor(ply / 2) + 1}${mover === "b" ? "…" : "."}</strong>
          <span>${mv.san}</span>
        </div>
        <div class="analysisMoveMeta">${mover === "w" ? "White" : "Black"} • CPL ${Math.round(cpl)}</div>
      </div>
      <div class="analysisMoveNote">${note}</div>
    `;

    card.addEventListener("click", () => setReviewPly(ply + 1));
    rows.push(card);
  }

  ui.analysisMoves.innerHTML = "";
  rows.forEach((r) => ui.analysisMoves.appendChild(r));
...
async function engineMoveIfNeeded() {
  ...
  try {
    const bm = await engine.bestMove(chess.fen(), { movetimeMs: PLAY_MOVE_TIME_MS });
  ...
  board.enableMoveInput((event) => {
    ...
    switch (event.type) {
      case INPUT_EVENT_TYPE.moveInputStarted: {
        // Only allow selecting your own pieces when it's your turn.
      ...
      case INPUT_EVENT_TYPE.validateMoveInput: {
        ...
        const ok = tryUserMove(event.squareFrom, event.squareTo, event.promotion);
